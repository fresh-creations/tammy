...
class PromptHandler:
                    ...
                    if parameter.startswith("image_prompt:"):
                        try:
                            image_prompts_series_dict[parameter] = get_inbetweens(
                                parameter_dicts[parameter], max_frames
                            )
                        except RuntimeError:
                            raise RuntimeError(
                                "WARNING: You have selected to use key frames, but you have not "
    ...
    def handle_interpolation(
        ...
        if key_frames:
            ...
            try:
                guidance_scale_series = get_inbetweens(parameter_dicts["guidance_scale"], max_frames, integer=False)
            except RuntimeError:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
                    "formatted `iterations_per_frame` correctly for key frames.\n"
            ...
            try:
                prompt_strength_series = get_inbetweens(parameter_dicts["prompt_strength"], max_frames, integer=False)
            except RuntimeError:
                print(
                    "WARNING: You have selected to use key frames, but you have not "
                    "formatted `iterations_per_frame` correctly for key frames.\n"
