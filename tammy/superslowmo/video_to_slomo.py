        ...
        with torch.no_grad():
            for _, (frame0, frame1) in enumerate(tqdm(slomoloader)):
                ...
                for batchIndex in range(self.batch_size):
                    (TP(frame0[batchIndex].detach())).resize(orig_dim, Image.BILINEAR).save(
                        os.path.join(outputPath, f"{frameCounter + self.slowmo_factor * batchIndex:06d}.png")
                    )
                frameCounter += 1
                ...
                for intermediateIndex in range(1, self.slowmo_factor):
                    ...
                    for batchIndex in range(self.batch_size):
                        (TP(Ft_p[batchIndex].cpu().detach())).resize(orig_dim, Image.BILINEAR).save(
                            os.path.join(outputPath, f"{frameCounter + self.slowmo_factor * batchIndex:06d}.png")
                        )
                    frameCounter += 1
